...
textLabel.Text = "Legacy Police Config Loaded!\nCoded by Ege\nShoutout to Vexanair"
...
game:GetService("StarterGui"):SetCore("SendNotification", {
    Title = "Criminal Aimbot",
    Text = "Hold Right Click to lock onto Criminals.",
    Duration = 99999
})
...
local SMOOTHNESS = 10
local ESP_TEAM_NAME = "Criminal" -- Changed from "Criminal"
...
local function addESP(player)
	...
	if player.Team and player.Team.Name == ESP_TEAM_NAME then
		...
		if character and character:FindFirstChild("HumanoidRootPart") then
			...
			highlight.FillColor = Color3.fromRGB(255, 0, 0) -- Blue
			...
			nameLabel.TextColor3 = Color3.new(1, 0, 0) -- Blue
...
local function getClosestTargetInFOV()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Team and player.Team.Name == "Criminal" and player.Character and player.Character:FindFirstChild("Head") then
